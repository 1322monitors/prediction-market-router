...
A working example that watches a live X/Twitter feed for keyword or phrase
matches and forwards each match to a webhook, as a small building block for a
prediction-market-style event pipeline.

It connects to the [1322](https://1322.io) X/Twitter WebSocket, checks every
incoming tweet against a keyword list you configure, dedupes by tweet id, and
POSTs a normalized JSON event to a webhook URL you configure. That's the
whole scope: **this is a generic signal router, not an integration with any
specific prediction-market platform.** Point `WEBHOOK_URL` at whatever you
use downstream - a queue, a serverless function, your own resolution
pipeline, Zapier/n8n, or a platform's API if you've built that integration
yourself.
...
Other real-time monitoring examples from the same author:
...
- [truthsocial-stream](https://github.com/SisoSol/truthsocial-stream)
